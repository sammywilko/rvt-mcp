...
.DESCRIPTION
  Runs a 4-step sweep:
    ...
    3. MCP host config entries matching bimwright-rvt or legacy bimwright-rvt-* in:
...
function Remove-ClaudeCodeGlobalEntries {
    ...
    foreach ($cfgPath in $candidates) {
        ...
        $bimKeys = @($cfg['mcpServers'].Keys | Where-Object { $_ -eq 'bimwright-rvt' -or $_ -like 'bimwright-rvt-*' })
...
function Remove-OpencodeEntries {
    ...
    $bimKeys = @($cfg['mcp'].Keys | Where-Object { $_ -eq 'bimwright-rvt' -or $_ -like 'bimwright-rvt-*' })
