...
Setup ZIP có sẵn `bimwright-rvt.exe` self-contained, nên máy client không cần `.NET 8 SDK`, `dotnet tool install`, hoặc repo này. Config MCP dùng absolute path đã cài, không phụ thuộc `%USERPROFILE%\.dotnet\tools` hay PATH.
